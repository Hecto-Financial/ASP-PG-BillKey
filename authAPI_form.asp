 ...
 */
 function validationCheck(){
     ...
     if (document.STPG_payForm.elements["cardPwd"].value.length != 2){ // cardPwd의 길이가 2가 아니면 false 반환
...
/** Submit버튼 동작 */
function doAction(){
    $('#STPG_payForm').attr("action", "authAPI_showResult.asp");
	$('#STPG_payForm').attr("method", "post");
	$('#STPG_payForm').attr("target", "_self");
	if ( validationCheck() ) $('#STPG_payForm').submit();
}
</script>
</head>
<body>
...
<form id="STPG_payForm" name="STPG_payForm" >
    ...
    <tr class="required">
        <td>mchtId[상점아이디]</td><td><input type="text" name="mchtId" value="<%= PG_MID %>" maxlength="12"/></td>
    </tr>
