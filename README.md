...
- **pay_showResult.asp**: 헥토파이낸셜과 Server to Server로 커넥션하여, 결제 요청을 하고 응답을 받아 결과를 출력하는 페이지입니다.
- **authAPI_form.asp**: 빌키 발급 API 양식으로서, 결제하지 않고 응답으로 빌키가 발급됩니다.
- **authAPI_showResult.asp**: 헥토파이낸셜과 Server to Server로 커넥션하여, 요청을 하고 응답을 받아 결과를 출력하는 페이지입니다.
